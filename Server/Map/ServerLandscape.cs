...
public sealed partial class ServerLandscape : BaseLandscape, IDisposable, ILogging
{
    ...
    public void SetLandZ(CEDServer server, ushort x, ushort y, sbyte newZ)
    {
        ...
        foreach (var ns in GetBlockSubscriptions((ushort)(x / 8), (ushort)(y / 8)))
        {
            ns.Send(packet);
        }
    }
    ...
    public void SetStaticZ(CEDServer server, StaticTile tile, sbyte newZ)
    {
        ...
        block.SortTiles(ref TileDataProvider.StaticTiles);
        foreach (var ns in GetBlockSubscriptions(block.X, block.Y))
        {
            ns.Send(packet);
        }
    }
        ...
        for (ushort x = x1; x <= x2; x++)
        {
            for (ushort y = y1; y <= y2; y++)
            {
                ...
                foreach (var staticTile in staticBlock.GetTiles(x, y).ToArray())
                {
                    ...
                    foreach (var ns in GetBlockSubscriptions(staticBlock.X, staticBlock.Y))
                    {
